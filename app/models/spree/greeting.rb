# GREETINGS
# Greetings represent an entity for sale in a store.
# Greetings can have variations, called variants
# Greetings properties include description, permalink, availability,
#   shipping category, etc. that do not change by variant.
#
# MASTER VARIANT
# Every greeting has one master variant, which stores master price and sku, size and weight, etc.
# The master variant does not have option values associated with it.
# Price, SKU, size, weight, etc. are all delegated to the master variant.
# Contains on_hand inventory levels only when there are no variants for the greeting.
#
# VARIANTS
# All variants can access the greeting properties directly (via reverse delegation).
# Inventory units are tied to Variant.
# The master variant can have inventory units, but not option values.
# All other variants have option values and may have inventory units.
# Sum of on_hand each variant's inventory level determine "on_hand" level for the Greeting.
#
...
module Spree
  class Greeting < Spree::Base
    ...
    has_many :greeting_option_types, dependent: :destroy, inverse_of: :greeting
    has_many :option_types, through: :greeting_option_types
    #has_many :greeting_properties, dependent: :destroy, inverse_of: :greeting
    #has_many :properties, through: :greeting_properties

    has_many :greeting_classifications, dependent: :delete_all, inverse_of: :greeting
    has_many :taxons, through: :greeting_classifications, before_remove: :remove_taxon

    has_many :greeting_promotion_rules, class_name: 'Spree::GreetingPromotionRule'
    has_many :promotion_rules, through: :greeting_promotion_rules, class_name: 'Spree::PromotionRule'
    ...
    belongs_to :shipping_category, class_name: 'Spree::ShippingCategory', inverse_of: :greetings
    ...
    #accepts_nested_attributes_for :greeting_properties, allow_destroy: true, reject_if: lambda { |pp| pp[:property_name].blank? }

    alias :options :greeting_option_types
    ...
    # Ensures option_types and greeting_option_types exist for keys in option_values_hash
    def ensure_option_types_exist_for_values_hash
      ...
      missing_option_type_ids.each do |id|
        greeting_option_types.create(option_type_id: id)
      end
    end

    # for adding greetings which are closely related to existing ones
    # define "duplicate_extra" for site-specific actions, eg for additional fields
    def duplicate
      duplicator = GreetingDuplicator.new(self)
    ...
    # determine if greeting is available.
    # deleted greetings and greetings with nil or future available_on date
    # are not available
    def available?
    ...
    def self.like_any(fields, values)
      conditions = fields.greeting(values).map do |(field, value)|
...
=begin
    def property(property_name)
      greeting_properties.joins(:property).find_by(spree_properties: { name: property_name }).try(:value)
    end
    ...
    def set_property(property_name, property_value, property_presentation = property_name)
      ActiveRecord::Base.transaction do
        ...
        greeting_property = GreetingProperty.where(greeting: self, property: property).first_or_initialize
        greeting_property.value = property_value
        greeting_property.save!
      end
    end
=end
    ...
    # Master variant may be deleted (i.e. when the greeting is deleted)
    ...
    def add_associations_from_prototype
      if prototype_id && prototype = Spree::Prototype.find_by(id: prototype_id)
        ...
        #  greeting_properties.create(property: property)
        #end
    ...
    def build_variants_from_option_values_hash
      ...
      values = values.inject(values.shift) { |memo, value| memo.greeting(value).map(&:flatten) }
    ...
    # If the master cannot be saved, the Greeting object will get its errors
    ...
    # Get the taxonomy ids of all taxons assigned to this greeting and their ancestors.
    def taxonomy_ids
      taxon_and_ancestors.map(&:taxonomy_id).flatten.uniq
    end

    # Iterate through this greetings taxons and taxonomies and touch their timestamps in a batch
    def touch_taxons
    ...
    def remove_taxon(taxon)
      removed_greeting_classifications = greeting_classifications.where(taxon: taxon)
      removed_greeting_classifications.each &:remove_from_list
    end
  end
end

require_dependency 'spree/greeting/scopes'
